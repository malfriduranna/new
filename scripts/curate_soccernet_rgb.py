...
def _load_events(labels_path: Path, keep_classes: Sequence[str], competition: str) -> List[Event]:
    ...
    for game in games:
        if competition and game.get("competition") and game["competition"] != competition:
            continue
        game_id = game.get("game") or game.get("id") or game.get("match") or ""
        if not game_id:
            raise ValueError("Could not find a game identifier in labels file.")
...
def curate(
    ...
    *,
    ...
    print(f"Finished. Clips per class: {summary}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Curate SoccerNet RGB clips for VideoMAE.")
    parser.add_argument("--labels-json", type=Path, required=True, help="Path to Labels-v2.json")
    parser.add_argument("--videos-root", type=Path, required=True, help="Root directory containing 720p videos.")
    parser.add_argument("--output-root", type=Path, default=Path("dataset"), help="Directory to write curated clips.")
    parser.add_argument(
        "--classes",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional format string to locate videos (e.g., '{game_id}_{half}.{ext}').",
    )
